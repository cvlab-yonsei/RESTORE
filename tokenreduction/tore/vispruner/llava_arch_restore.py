...
def encode_images(self, images):
    image_features, image_attentions = self.get_model().get_vision_tower()(images) # (B, N, C), (B, H, N)
    
    B, N, C = image_features.shape
    device = image_features.device
    index_masks = torch.ones(B, N, dtype=torch.bool, device=device)

    # visual_token_num = self.get_visual_token_num() # T --> self.n_retain
    # important_ratio = self.get_important_ratio() # r
    important_token_num = int(self.n_retain * self.important_ratio) # T_imp = T * r
    diverse_token_num = self.n_retain - important_token_num # T_div = T * (1 - r)
    ...
    index_masks = torch.zeros(B, N, dtype=torch.bool, device=device) # (B, N), (1, 576)
...
def prepare_inputs_labels_for_multimodal(
    ...
    else: # we only this case - a single image tensor
        image_features, index_masks = self.encode_images(images) # (B, N, D), (B, N)
    ...
    retained_indices = index_masks.to(dtype=torch.long).nonzero(as_tuple=False)[:, 1].view(B, n_retain)  # (B, 32)
    pruned_indices = (~index_masks).to(dtype=torch.long).nonzero(as_tuple=False)[:, 1].view(B, N_vis - n_retain)  # (B, 544)
    ...
    image_features_retained = image_features[index_masks].unsqueeze(0)
    
    if self.restore==True: # need merging_pattern
        image_features_to_merge = image_features[:, ~index_masks[0], :]
